...
- **P7 阶段** ✅ 已完成 — 符文系统（5个C#文件）
- **P8 阶段** ✅ 已完成 — 觉醒与成长加速系统（4个C#文件）
- **P9 阶段** ✅ 已完成 — 幸存者系统（5个C#文件）
- **P10 阶段** ⏳ 待开始 — 数据存储与存档系统
- ...（后续阶段按执行计划书推进）
...
| 文件 | 说明 |
|------|------|
| `RuneEnums.cs` | 符文分类/特殊效果/槽位类型枚举 |
| `RuneData.cs` | 符文配置ScriptableObject + 符文实例类 |
| `RuneManager.cs` | 符文管理器，6槽位，装备/抽卡/升星/碎片 |
| `RuneFactory.cs` | 符文配置工厂，12种符文（攻击/防御/生存各4个） |

### 测试
| 文件 | 说明 |
|------|------|
| `RuneSystemTests.cs` | 符文系统单元测试（20+测试用例） |

### P7 核心特性
- ✅ 12种符文：攻击/防御/生存各4个（白/蓝/紫/金各3个）
- ✅ 主属性 + 副属性（紫+） + 特殊效果（金）
- ✅ 6个符文槽位：2攻击+2防御+1生存+1特殊
- ✅ 最高5星，每星+20%属性成长
- ✅ 碎片系统：升星消耗碎片，碎片合成完整符文
- ✅ 抽卡系统：白60%/蓝30%/紫8%/金2%
- ✅ 三重保底：10连必蓝/50连必紫/100连必金
- ✅ 软保底：连续3次同品质同类型则换类型
- ✅ 属性通过 Pct_Rune 层生效，事件驱动刷新
- ✅ 完整单元测试覆盖

## P8 阶段文件清单

### 觉醒与成长加速系统（Logic/Awakening）
| 文件 | 说明 |
|------|------|
| `AwakenManager.cs` | 觉醒管理器，100级，AXP经验，全局属性倍率 |
| `SpeedupManager.cs` | 成长加速管理器，连败/卡关/新手保护Buff |
| `DDDRSystem.cs` | DDDR动态难度调节，实时检测平滑过渡 |

### 测试
| 文件 | 说明 |
|------|------|
| `AwakeningSystemTests.cs` | 觉醒与加速系统单元测试（25+测试用例） |

### P8 核心特性
- ✅ 觉醒系统：最高100级，每级+2.5%全属性，满级3.5x
- ✅ AXP经验：分段需求（1k→3k→6k→12k→20k）
- ✅ 成长加速：5种加速Buff（连败/卡关/卡天/新手/周末）
- ✅ 连续失败3次 → +20%全属性
- ✅ 同一波次卡5次 → +25%全属性
- ✅ 新手保护（前3天） → +30%全属性
- ✅ Buff叠加：多个Buff倍率相乘
- ✅ DDDR动态难度：每0.5秒检测，平滑过渡
- ✅ 三重调节：生成速率/丧尸攻击/丧尸速度
- ✅ 难度范围：-40% ~ +50%
- ✅ 事件驱动：升级/Buff变化/难度变化全部事件化
- ✅ 完整单元测试覆盖

## P9 阶段文件清单

### 幸存者系统（Logic/Survivors）
| 文件 | 说明 |
|------|------|
| `SurvivorEnums.cs` | 职业/稀有度/状态/技能类型枚举 |
| `SurvivorData.cs` | 幸存者配置ScriptableObject + 实例类 |
| `SurvivorFactory.cs` | 幸存者配置工厂，12个幸存者（6职业×4品质） |
| `SurvivorManager.cs` | 幸存者管理器，招募/上阵/升级/战斗生命周期 |
| `SurvivorAIController.cs` | 幸存者战斗AI，6状态状态机 |

### 测试
| 文件 | 说明 |
|------|------|
| `SurvivorSystemTests.cs` | 幸存者系统单元测试（25+测试用例） |

### P9 核心特性
- ✅ 12个幸存者：6大职业（战士/射手/医生/工程师/侦察/守卫）
- ✅ 4种稀有度：白/蓝/紫/金，各3个
- ✅ 每个幸存者最多3个技能（主动+被动+工作）
- ✅ 最高50级，属性百分比成长
- ✅ 招募系统：单抽+十连，三重保底（10连必蓝/30连必紫/100连必金）
- ✅ 上阵系统：最多3人同时出战
- ✅ 战斗AI：6状态（待机/追击/攻击/逃跑/辅助/施法）
- ✅ 职业差异化：近战/远程/治疗/防御/辅助
- ✅ 受伤系统：血量归零进入受伤状态，休息后恢复
- ✅ 亲密度系统（预留扩展）
- ✅ 事件驱动：招募/上阵/升级全部事件化
- ✅ 完整单元测试覆盖
